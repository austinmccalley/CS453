...
void keyboard(unsigned char key, int x, int y)
{
  ...
  case '4':
    ...
    for (int i = 0; i < poly->nverts; i++)
    {
      ...
      double val = (tempV->scalar - poly->minScalar) / (poly->maxScalar - poly->minScalar);
      tempV->R = tempV->G = tempV->B = val;
    }
  ...
  case '5':
    ...
    for (int i = 0; i < poly->nverts; i++)
    {
      ...
      tempV->R = 1 * (tempV->scalar - poly->minScalar) / (poly->maxScalar - poly->minScalar) +
          1 * (poly->maxScalar - tempV->scalar) / (poly->maxScalar - poly->minScalar);
      tempV->G = 1 * (tempV->scalar - poly->minScalar) / (poly->maxScalar - poly->minScalar) +
          0 * (poly->maxScalar - tempV->scalar) / (poly->maxScalar - poly->minScalar);
      tempV->B = 0 * (tempV->scalar - poly->minScalar) / (poly->maxScalar - poly->minScalar) +
          1 * (poly->maxScalar - tempV->scalar) / (poly->maxScalar - poly->minScalar);
    }
    display();
    break;
...
void display_polyhedron(Polyhedron *poly)
{
  ...
  for (i = 0; i < poly->nquads; i++)
  {
    ...
    case 4:
      ...
      for (j = 0; j < 4; j++)
      {
        Vertex *tempV = temp_q->verts[j];
        glColor3f(tempV->R, tempV->G, tempV->B);
        glVertex3d(tempV->x, tempV->y, tempV->z);
      }
      glEnd();
      break;
    case 5:
