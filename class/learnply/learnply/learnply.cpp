...
void keyboard(unsigned char key, int x, int y)
{
	...
	break;
	case '4': // random grayscale display
	{
		display_mode = 4;
		//codes to modify the color of each vertex

		// give each vertex a rand gray scale value
		for (int i = 0; i < poly->nverts; i++)
		{
			Vertex *temp_v = poly->vlist[i];
			double val = (double)rand() / (double)RAND_MAX;

			temp_v->R = temp_v->G = temp_v->B = val;
		}
		display();
		break;
	}
	...
	case '5': // rand color
	{
		...
		{
			Vertex *temp_v = poly->vlist[i];
			temp_v->R = (double)rand() / (double)RAND_MAX;
			temp_v->G = (double)rand() / (double)RAND_MAX;
			temp_v->B = (double)rand() / (double)RAND_MAX;
			;
		}
		display();
		break;
	...
	case '6': // z-pos = r^2
	{
		...
		// Get center
		icVector2 center = icVector2(poly->center.x, poly->center.y);
		...
		for (int i = 0; i < poly->nverts; i++)
		{
			...
			icVector2 pos = icVector2(temp_v->x, temp_v->y);
			icVector2 diff = pos - center;

			double squaredDiff = length(diff) * length(diff);

			temp_v->z = -0.1 * squaredDiff;
		}
		display();
		break;
	}
	case '7':
	{ // Color each face based on the length of its perimeter
		display_mode = 7;
...
void mouse(int button, int state, int x, int y)
{
	...
	if (button == GLUT_LEFT_BUTTON || button == GLUT_RIGHT_BUTTON)
	{
		...
		else if (state == GLUT_UP)
		{
			...
			if (button == GLUT_LEFT_BUTTON && key == GLUT_ACTIVE_SHIFT)
...
void display(void)
{
	glClearColor(0.5, 0.5, 0.5, 0.5); // background for rendering color coding and lighting
...
void display_polyhedron(Polyhedron *poly)
{
	...
	for (i = 0; i < poly->nquads; i++)
	{
		...
		case 6:
		{
			...
			glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
			GLfloat mat_diffuse[4] = {1.0, 0.2, 0.8, 0.0};
			GLfloat mat_specular[] = {1.0, 1.0, 1.0, 1.0};
			glMaterialfv(GL_FRONT, GL_DIFFUSE, mat_diffuse);
			glMaterialfv(GL_FRONT, GL_SPECULAR, mat_specular);
			glMaterialf(GL_FRONT, GL_SHININESS, 50.0);
		...
		case 7:
			...
			// set color according to perimeter len
			double val = (temp_q->perim - poly->min_perim) / (poly->max_perim - poly->min_perim);

			glColor3d(0.0, val, val);
			glBegin(GL_POLYGON);
			for (j = 0; j < 4; j++)
			{
				Vertex *temp_v = temp_q->verts[j];
